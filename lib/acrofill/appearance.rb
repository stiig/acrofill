...
module Acrofill
  ...
  class Appearance
    ...
    # Multiline uses a flat 12pt instead, which is deliberate.
    def auto_size(size, height)
      size.zero? ? [height * 0.66, DEFAULT_SIZE].min : size
    end
    ...
    def wrap_line(text, font, size, max_width)
      space = font.width_of(' ', size)
      lines = ['']
      so_far = [0.0]
      text.split.each do |word|
        word_width = font.width_of(word, size)
        if lines.last.empty?
          lines[-1] = word
          so_far[-1] = word_width
